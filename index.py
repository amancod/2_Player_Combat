...
def instructions():
    while True:
        ...
        CONTENT_TEXT = get_font(30).render(
            "1.) This is a two player fighter game ,one fighter being WARRIOR and the other WIZARD.", True, "white")
        CONTENT_RECT = CONTENT_TEXT.get_rect(center=(500, 250))
        screen.blit(CONTENT_TEXT, CONTENT_RECT)
        # player 1
        WARRIOR_TEXT = get_font(30).render(
            "2.) The key controls for warrior are: a*left  d*right  w*jump  z*attack1  x*attack2   ", True, "white")
        WARRIOR_RECT = WARRIOR_TEXT.get_rect(center=(500, 300))
        screen.blit(WARRIOR_TEXT, WARRIOR_RECT)
        # player 2
        WIZARD_TEXT = get_font2(30).render(
            "3.)  The key controls for wizard are: 4*left  6*right  8*jump  1*attack1  2*attack2     ", True, "white")
        WIZARD_RECT = WIZARD_TEXT.get_rect(center=(500, 350))
        screen.blit(WIZARD_TEXT, WIZARD_RECT)

        CONTENT_TEXT2 = get_font(30).render(
            "4.) Each bout is of three rounds and the player with maximum points wins the bout.   ", True, "white")
...
def main_menu():
    while True:
        ...
        INSTRUCTIONS_BUTTON = Button(image=None, pos=(500, 525),
                                     text_input="INSTRUCTIONS", font=get_font(75), base_color="#d7fcd4",
